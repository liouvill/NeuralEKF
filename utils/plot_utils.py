...
DEFAULT_ENDMARK_LIST = ("rx", "bx", "yx", "ox", "px")
DEFAULT_COLOR_LIST = ("r", "b", "y", "o", "p")
DEFAULT_LINEWIDTH_LIST = (5, 5, 1, 1)
DEFAULT_LABEL_LIST = ("ground-truth", "ground-truth", "prediction", "prediction")
...
class _PlotHandler:
    ...
    def plot_timeseries_compare(
        ...
        endmark_list: Sequence[Optional[str]] = DEFAULT_ENDMARK_LIST,
        xlabel: str = "t",
        color_list: Sequence[Optional[str]] = DEFAULT_COLOR_LIST,
        linewidth_list: Sequence[Optional[int]] = DEFAULT_LINEWIDTH_LIST,
        label_list: Sequence[Optional[int]] = DEFAULT_LABEL_LIST,
    ) -> None:
    ...
    def plot_timeseries(
        ...
        # start/end marks
        if startmark is not None:
            plt.plot(time[0] * np.ones(B), data[0, ...], color=color, marker=startmark, linestyle="None")
        if endmark is not None:
            plt.plot(time[-1] * np.ones(B), data[-1, ...], color=color, marker=endmark, linestyle="None")

        # linestyle
        if style is None:
            plt.plot(time, data)
        if label is None:
            plt.plot(time, data, color=color, linestyle=style, linewidth=linewidth)
        else:
            plt.plot(time, data, color=color, linestyle=style, linewidth=linewidth, label=label)
            plt.legend(loc="upper right", ncol= 1, prop={'family' : 'Times New Roman', 'size' : 10})

        # labels
        plt.xlabel(xlabel)
        plt.ylabel(ylabel)

        #plt.fill_between(time, data[:,0]-np.sqrt(cov[:,0]), data[:,0]+np.sqrt(cov[:,0]), facecolor='yellow', alpha=0.6, label='2 sigma range')
